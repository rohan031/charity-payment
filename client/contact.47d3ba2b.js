...
function animationBasedOnDevice(media, ob) {
    ...
    if (media.matches) {
        ...
        let observer = new IntersectionObserver((entries, observer)=>{
            entries.forEach((entry)=>{
                if (entry.isIntersecting) gsap.to(".cards-list__item", {
                    x: 0,
                    opacity: 1,
                    duration: 1.4,
                    stagger: 0.3,
    ...
    let observer1 = new IntersectionObserver((entries, observer)=>{
        entries.forEach((entry)=>{
            if (entry.isIntersecting) gsap.to(entry.target, {
                x: 0,
                opacity: 1,
                duration: 1,
                stagger: 0.4,
    ...
    listItems.forEach((item)=>observer1.observe(item));
    return observer1;
}
// media queries
...
//# sourceMappingURL=contact.47d3ba2b.js.map
