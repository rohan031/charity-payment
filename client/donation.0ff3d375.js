...
terms.addEventListener("change", (e)=>{
    if (e.target.checked) termsError.innerHTML = "";
});
...
const displayDetails = (userInputs)=>{
    ...
    for(const prop in userInputs){
        if (i < 9) values[i].innerHTML = userInputs[prop];
        i++;
    }
    // adding loader
...
proceed.addEventListener("click", (event)=>{
    ...
    if (!terms.checked) {
        termsError.innerHTML = "You need to agree to our Terms and Conditions to proceed.";
        return;
    }
    if (validateUserInputs()) return;
    const fullName = title.value + " " + firstName.value.trim() + " " + lastName.value.trim();
    userInputs = {
        name: fullName,
...
proceedReview.addEventListener("click", ()=>{
    ...
    fetch("/paynow", options).then((res)=>res.json()).then((res)=>console.log(res)).catch((err)=>console.log(err));
});

//# sourceMappingURL=donation.0ff3d375.js.map
